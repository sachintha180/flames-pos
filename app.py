...
from sqlalchemy import Column, Table, DateTime, Integer, String, Numeric, func, ForeignKey, UniqueConstraint, Enum
...
class User(db.Model):
    ...
    role: Mapped[me.UserRole] = mapped_column(Enum(me.UserRole), nullable=False, server_default=me.UserRole.cashier.value)
...
class Customer(db.Model):
    ...
    registered_at: Mapped[str] = mapped_column(DateTime, nullable=False, server_default=func.now())
    # foreign key constraints for customer-order (1:M) - ON DELETE CASCADE
    order: Mapped[List["Order"]] = relationship(back_populates="customer", cascade="all, delete", passive_deletes=True)

class Staff(db.Model):
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    name: Mapped[str] = mapped_column(String, nullable=False)
    role: Mapped[me.StaffRole] = mapped_column(Enum(me.StaffRole), nullable=False, server_default=me.StaffRole.waiter.value)
    mobile_no: Mapped[str] = mapped_column(String(10), nullable=False, unique=True)
    email: Mapped[str] = mapped_column(String, unique=True)
...
class Payment(db.Model):
    ...
    ptype: Mapped[me.PaymentType] = mapped_column(Enum(me.PaymentType), nullable=False, server_default=me.PaymentType.cash.value)
...
class Order(db.Model):
    ...
    status: Mapped[me.OrderStatus] = mapped_column(Enum(me.OrderStatus), nullable=False, server_default=me.OrderStatus.pending.value)
    ordered_at: Mapped[datetime.datetime] = mapped_column(DateTime, nullable=False, server_default=func.now())
    ...
    product: Mapped[List["Product"]] = relationship(secondary=OrderProduct, back_populates="order")
...
class Product(db.Model):
    ...
    # foreign key constraint for order-product (1:M) - ON DELETE NO CHANGE
    order: Mapped[List["Order"]] = relationship(secondary=OrderProduct, back_populates="product")
